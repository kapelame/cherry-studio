/**
 * Lab registry — the ONLY hand-maintained source for the model catalog.
 *
 * `data/models.json` is generated (see `scripts/generate-catalog.ts`): each lab supplies its model
 * LIST (from its own API and/or hand-written entries), and metadata is enriched from
 * models.dev/OpenRouter. You never edit `models.json` by hand — add/override in a lab here instead.
 */
import type { ModelConfig } from '../schemas/model'

/**
 * A model a lab declares by hand. It is a Partial `ModelConfig` — `ownedBy` comes from the lab and
 ...
 */
export type LabModel = Partial<Omit<ModelConfig, 'ownedBy' | 'metadata'>> & { id: string }
...
export interface Lab {
  ...
  name: string
  /**
   * Most native source: fetch this lab's list from its OWN `/models` API, parsing its response shape
   * (each lab differs). Reads the lab's key from env; if the key is absent or the call fails, the
   * generator falls back to the models.dev fields below. See `./_api.ts` for shared helpers.
   */
  fetchModels?: () => Promise<LabModel[]>
  /** Fallback list source: models.dev provider key(s) whose listing is this lab's catalog. */
  ...
  idPrefixes?: string[]
  /**
   * Curated web-search capability as DATA (no `inferXxx`): canonical id-prefixes of THIS lab's models
  ...
  models?: LabModel[]
  /**
   * Default model kind for labs whose models are all embedders/rerankers but don't say so in their id
...
export function defineLab(lab: Lab): Lab {
  return lab
}
